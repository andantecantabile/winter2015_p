...
class MakeGUI(object):
    def __init__(self,root):
        ...
        ## build frame
        self.mainframe = tkinter.ttk.Frame(self.root, padding=(6, 6, 12, 12))
        ...
        ## text labels
        tkinter.ttk.Label(self.mainframe, text=u"Label Title", anchor='center',
                font=("Helvetica", 32)).grid(in_=self.mainframe,
                        column=0, row=0, columnspan=3, sticky="ew")

        self.lfdata = tkinter.ttk.Labelframe(self.mainframe, padding=(6, 6, 12, 12),
                text='Labelframe')
        ...
        for i, item in enumerate(info):
            tkinter.ttk.Label(self.lfdata, text=u"%s:" % item).grid(in_=self.lfdata,
                    column=0, row=i, sticky='w')

        ## buttons
        btn = (u"Close", u"Next", u"Prev")
        for i, item in enumerate(btn):
            tkinter.ttk.Button(self.mainframe, text=item).grid(in_=self.mainframe,
                    column=i, row=3)
